...
public abstract class AbstractRaffleStrategy implements IRaffleStrategy {
    ...
    public RaffleAwardEntity performRaffle(RaffleFactorEntity raffleFactorEntity) {
        ...
        // 2. 责任链抽奖计算【这步拿到的是初步的抽奖ID，之后需要根据ID处理抽奖】注意；黑名单、权重、保底等非默认抽奖的直接返回抽奖结果
        DefaultChainFactory.StrategyAwardVO chainStrategyAwardVO = raffleLogicChain(userId, strategyId, raffleFactorEntity.getTotalUserRaffleCount(), raffleFactorEntity.getEndDateTime());
        ...
        DefaultTreeFactory.StrategyAwardVO treeStrategyAwardVO = raffleLogicTree(userId, strategyId, chainStrategyAwardVO.getAwardId(),raffleFactorEntity.getEndDateTime(),raffleFactorEntity.getTodayUserRaffleCount());
     ...
     * @param totalUserRaffleCount 累计抽奖次数【10连抽保底使用，单抽为 null】
     * @param endDateTime          活动结束时间【保底扣库存加锁使用】
     * @return 奖品ID
     */
    public abstract DefaultChainFactory.StrategyAwardVO raffleLogicChain(String userId, Long strategyId, Long totalUserRaffleCount, Date endDateTime);
     ...
     * @param todayUserRaffleCount 当日抽奖次数【10连抽 rule_lock 使用，单抽为 null 时节点回退查库】
     * @return 过滤结果【奖品ID，会根据抽奖次数判断、库存判断、兜底兜里返回最终的可获得奖品信息】
     */
    public abstract DefaultTreeFactory.StrategyAwardVO raffleLogicTree(String userId, Long strategyId, Integer awardId, Date endDateTime, Integer todayUserRaffleCount);
